...
export function generarReporteMezclaPdf(mezcla) {
...
export function generarReporteMezclaPdfBlob(mezcla) {
  const doc = generarReporteMezclaPdf(mezcla);
  const nombre = `Reporte-Prueba-${mezcla.codigo || mezcla.uuid}.pdf`;
  return { blob: doc.output("blob"), nombre };
}

export function descargarReporteMezclaPdf(mezcla) {
  const doc = generarReporteMezclaPdf(mezcla);
...
export function verReporteMezclaPdf(mezcla) {
  const doc = generarReporteMezclaPdf(mezcla);
