...
// El backend manda el detalle de qué campo falló en `errors` (ej. validate.middleware.js
// / errorHandler.middleware.js: [{ field, message }]), pero antes se descartaba y solo
// se mostraba el mensaje genérico ("Error de validación") — sin decir cuál campo.
function mensajeDeError(data) {
  const base = data?.message || "Error en la petición";
  if (Array.isArray(data?.errors) && data.errors.length > 0) {
    const detalle = data.errors.map((e) => (e.field ? `${e.field}: ${e.message}` : e.message)).join(" — ");
    return `${base} (${detalle})`;
  }
  return base;
}
...
// Igual que apiFetch, pero para enviar un FormData (multipart) con texto y
// archivos mezclados en un solo POST — ej. Comunicados con adjuntos. No se
// fija Content-Type: el navegador arma el boundary correcto solo.
